  ...
  try {
    ...
    for (const row of rows) {
      ...
      const membership = membershipByKey.get(
        membershipKey(recipient, row.tenant_id)
      ) as
        | {
            user_email: string;
      ...
      const wantEmail = membership.is_active !== false;
      const wantPush =
        membership.receive_push === true && hasToken.has(recipient.toLowerCase());
      ...
      if (shouldSendEmail) {
        ...
        let textBody: string;
        let htmlBody: string;
        const template = templateByTenant.get(row.tenant_id);
        if (template) {
