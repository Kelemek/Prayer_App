-- Tenant-scoped prayer encouragement settings (admin MFA + public read for Pray For UI).
...
-- Public read for prayer cards (no auth required).
...
grant execute on function public.update_tenant_prayer_encouragement_settings(uuid, boolean, integer, text) to anon, authenticated;
