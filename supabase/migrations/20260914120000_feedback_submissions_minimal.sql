-- In-app feedback: PII in Postgres only; Notion gets Submission ID (minimal pattern).
...
create table if not exists public.feedback_submissions (
  ...
  feedback_type text not null check (feedback_type in ('bug', 'feature', 'suggestion')),
...
create index if not exists idx_feedback_submissions_auth_user_id
  on public.feedback_submissions (auth_user_id);

create index if not exists idx_feedback_submissions_user_email_lower
  on public.feedback_submissions (lower(trim(user_email)));

comment on table public.feedback_submissions is
  'Contact info for in-app feedback. Notion tasks reference id only; resolve email via get_feedback_submission_contact (service role).';
...
create or replace function public.get_feedback_submission_contact(p_submission_id uuid)
returns jsonb
language plpgsql
security definer
...
begin
  if p_submission_id is null then
    raise exception 'submission_id required';
  end if;
  ...
  return jsonb_build_object(
    ...
    'feedback_type', v_row.feedback_type,
    'created_at', v_row.created_at
  );
end;
$$;

revoke all on function public.get_feedback_submission_contact(uuid) from public, anon, authenticated;
grant execute on function public.get_feedback_submission_contact(uuid) to service_role;


-- Re-apply erasure with feedback_submissions cleanup (after table exists).
...
begin
  ...
  where auth_user_id = p_user_id
     or lower(trim(user_email)) = v_email;
  get diagnostics v_n = row_count;
  v_counts := v_counts || jsonb_build_object('feedback_submissions_deleted', v_n);
