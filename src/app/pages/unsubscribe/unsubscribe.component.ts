      ...
      <div class="max-w-md w-full mx-auto space-y-8 p-4 sm:p-8 text-center">
        ...
        >
          ...
          >
            ...
            >
            <p
              class="text-xs text-[#3a7566] dark:text-emerald-200/90 max-w-[280px]"
            >
        ...
        <p class="text-gray-600 dark:text-gray-400 mb-6">
          You won’t receive these emails anymore. You can turn email
          notifications back on anytime in the app settings.
        </p>
        } @if (state() === 'error') {
        <h1 class="text-2xl font-semibold mb-3">Couldn’t unsubscribe</h1>
        <p class="text-gray-600 dark:text-gray-400 mb-4">
          This link may be invalid or expired. Try signing in and updating your
          email preferences in Settings.
        </p>
        @if (fallbackUnsubscribeUrl()) {
        <p class="text-sm text-gray-500 dark:text-gray-400 mb-4">
          You can also try completing unsubscribe on our mail server (opens a
        ...
        <p class="text-gray-600 dark:text-gray-400 mb-6">
          Open the unsubscribe link from your email, or sign in and change your
...
export class UnsubscribeComponent implements OnInit {
  ...
  readonly state = signal<"loading" | "success" | "error" | "missing">(
    "loading"
  );
  ...
  ngOnInit(): void {
    ...
    const fnUrl = `${baseUrl}/functions/v1/email-unsubscribe?token=${encodeURIComponent(
      token
    )}`;
