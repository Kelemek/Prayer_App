...
@Component({
  ...
  template: `
    <app-admin-collapsible-section
      title="Prayer Editor"
      triggerId="prayer-editor-trigger"
      panelId="prayer-editor-panel"
    ...
    >
      ...
      @if (isLoading) {
        <app-admin-section-loading message="Loading prayers…" />
      } @else {
        <div #prayerEditorContainer>
      <p class="text-sm text-gray-600 dark:text-gray-300 mb-4">
        Search and filter prayers by title, requester, email, description,
        denial reasons, or prayer update content. Use dropdown filters to
        automatically load results. Delete individually or in bulk.
      </p>
          ...
          >
            <div class="space-y-2">
              ...
              >
                ...
                <div class="space-y-3">
                  @for (update of prayer.prayer_updates; track update.id; let i
                  = $index) {
                  <div
                    [class]="
                      'p-3 rounded border ' +
      ...
      <!-- Empty States -->
      @if (!searching && allPrayers.length === 0 && searchTerm && !statusFilter
      && !approvalFilter) {
      ...
      <div class="text-center py-8 text-gray-500 dark:text-gray-400">
        ...
        <div class="text-sm mt-2 space-y-1">
          ...
          <p>
            • Or search by title, requester, email, description, prayer updates,
            or denial reasons
          </p>
...
export class PrayerSearchComponent implements OnInit, OnDestroy {
  ...
  /** Commas break PostgREST `or` filters; LIKE wildcards are escaped for the literal portion. */
  private escapeIlikeTerm(raw: string): string {
  ...
  private buildPrayersSelectQuery(
    tenantId: string,
    options: { applyTextOrFilter: boolean }
  ) {
    const select =
  ...
  async handleSearch(): Promise<void> {
    try {
      ...
      if (!tenantId) {
        ...
        this.currentPage = 1;
        this.loadPageData();
        this.error = "Select a tenant to search and edit prayers.";
        return;
      }

      const primary = await this.buildPrayersSelectQuery(tenantId, {
        applyTextOrFilter: true,
      });
      const { data: primaryData, error: primaryError } = await primary;
      if (primaryError) {
        throw new Error(`Query failed: ${primaryError.message}`);
      }
  ...
  async onConfirmDelete(): Promise<void> {
    try {
      ...
      if (this.isStatusUpdateConfirmation) {
        ...
        const tenantId = this.getTenantId();
        if (!tenantId) {
          this.toast.error("No tenant selected.");
  ...
  async createPrayer(event: Event): Promise<void> {
    ...
    try {
      ...
      const tenantId = this.getTenantId();
      if (!tenantId) {
        this.toast.error("Select a tenant before creating a prayer.");
        return;
      }
