  ...
  template: `
    ...
    >
      ...
      @if (!activeTenantId && !loading) {
        ...
        >
          Select an organization above to view activity for that tenant.
        </p>
      }
      ...
      @if (!loading && activeTenantId && series.length > 0) {
...
export class SiteAnalyticsActivityChartComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private async load(): Promise<void> {
    ...
    try {
      this.series = await this.analytics.getPageViewTimeSeries(tenantId, this.preset);
    } finally {
      this.loading = false;
