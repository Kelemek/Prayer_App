...
describe('PrayerListBookletPrintComponent', () => {
  ...
  describe('onSectionToggle', () => {
    it('loads insert pages on first expand scoped to active tenant', async () => {
      ...
      const chain = mockSupabase.client.from.mock.results[0].value;
      expect(chain.select().eq).toHaveBeenCalledWith('tenant_id', 'tenant-1');
    });
  });
