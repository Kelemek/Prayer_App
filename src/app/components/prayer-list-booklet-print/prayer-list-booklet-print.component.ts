...
>
  ...
  >
    <p class="text-sm text-gray-600 dark:text-gray-400 max-w-2xl">
      ...
      On mobile, the native print sheet opens the layout; saddle-stitch duplex works best from a desktop browser.
    </p>
...
export class PrayerListBookletPrintComponent {
  ...
  async loadInsertPages(): Promise<void> {
    ...
    if (!tenantId) {
      this.insertPages = [];
      this.insertPagesError = 'Select an active organization to manage custom insert pages.';
      return;
    }
  ...
  private async saveNewInsertPage(
    ...
    const tenantId = this.getActiveTenantId();
    if (!tenantId) {
      this.insertPagesError = 'Select an active organization before adding insert pages.';
      this.insertPagesBusy = false;
      this.cdr.markForCheck();
      return;
    }
