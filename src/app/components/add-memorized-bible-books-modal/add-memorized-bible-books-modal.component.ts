...
export class AddMemorizedBibleBooksModalComponent {
  ...
  async handleAdd(): Promise<void> {
    ...
    try {
      ...
      if (result.ok) {
        this.toast.success(`Added ${bibleBooksReferenceLabel(this.scope)} to your memorization list.`);
        this.added.emit();
        this.onClose.emit();
      } else if (result.reason === 'duplicate') {
        this.toast.error(`${bibleBooksReferenceLabel(this.scope)} is already in your memorization list.`);
      } else {
