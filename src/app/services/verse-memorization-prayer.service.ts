...
import { buildMemorizeVerseAppLink, buildViewPrayerAppLink } from '../lib/email-notification-links';
...
export class VerseMemorizationPrayerService {
  ...
  async createVerseMemorizationPrayer(options: {
    ...
    const requester = branding.appTitle?.trim() || 'Church';
  ...
  async broadcastVerseMemorizationPrayerNotifications(
    ...
    await this.emailNotification.sendVerseMemorizationPrayerNotification({
      ...
      tenantId: this.tenantContext.getActiveTenant()?.id,
    });
