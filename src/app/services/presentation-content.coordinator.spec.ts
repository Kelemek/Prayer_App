...
function createHost(
  ...
  const state = {
    ...
    canAccessSharedContent: overrides.canAccessSharedContent ?? true,
    catalog,
    markForCheckCalls: 0,
  };
  ...
  const host: PresentationContentHost & { markForCheckCalls: number } = {
    ...
    get canAccessSharedContent() {
      return state.canAccessSharedContent;
    },
    set canAccessSharedContent(value: boolean) {
      state.canAccessSharedContent = value;
    },
    markForCheck() {
