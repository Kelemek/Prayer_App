...
describe('AdminDataService', () => {
  ...
  describe('approveAccountRequest', () => {
    ...
    it('should finish approval even if sending email never resolves', async () => {
      ...
      mockEmailNotificationService.sendEmail = vi.fn(() => new Promise(() => {}));
      mockEmailNotificationService.sendSubscriberWelcomeNotification = vi.fn(
        () => new Promise(() => {})
      );

      await expect(service.approveAccountRequest('1')).resolves.toBeUndefined();
    });
  });
