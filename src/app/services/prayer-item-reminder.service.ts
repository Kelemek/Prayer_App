...
const SELECT_COLS =
  'id, tenant_id, user_email, prayer_kind, prayer_id, title_snapshot, prayer_for_snapshot, mode, iana_timezone, local_hour, local_minute, local_date, local_weekday, last_sent_at, created_at';
...
export class PrayerItemReminderService {
  ...
  async addReminder(
    ...
    const session = this.userSession.getCurrentSession();
    const cached = session?.prayerItemReminders;
    if (cached?.some((r) => prayerItemReminderSchedulesMatch(r, input))) {
