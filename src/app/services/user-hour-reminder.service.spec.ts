...
function asOrderQuery<T>(value: T): T & { eq: ReturnType<typeof vi.fn> } {
  const result = Promise.resolve(value) as T & { eq: ReturnType<typeof vi.fn> };
...
describe('UserHourReminderService', () => {
  ...
  beforeEach(() => {
    selectOrderMock = vi.fn(() => asOrderQuery({
    ...
    const deleteFinalEq = vi.fn(() => Promise.resolve({ error: null }));
    const deleteTenantEq = vi.fn(() => deleteFinalEq);
    const deleteEmailEq = vi.fn(() => ({
      eq: deleteTenantEq,
    }));
  ...
  it('does not apply stale background fetch after addSlot invalidates generation', async () => {
    ...
    selectOrderMock.mockReturnValueOnce(asOrderQuery(backgroundPromise));

    void service.ensureLoaded('memorization', false);

    selectOrderMock.mockReturnValueOnce(
      asOrderQuery({
