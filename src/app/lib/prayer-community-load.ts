...
export function groupPrayerUpdatesByPrayerId(
  updatesData: Array<{ prayer_id: string }> | null | undefined
): Map<string, any[]> {
  ...
  return updatesByPrayerId;
}

/** Dest loadPrayers mapping: prayers from a flat select plus a second updates query. */
export function formatApprovedCommunityPrayersFromUpdatesMap(
  prayersData: Record<string, unknown>[],
  updatesByPrayerId: Map<string, any[]>
): PrayerRequest[] {
  const formatted = (prayersData || []).map((prayer) => {
    const updates = (updatesByPrayerId.get(prayer['id'] as string) || []).sort(
...
export function formatPrayersByMonthFromSeparateUpdates(
  prayersData: Record<string, unknown>[],
  updatesByPrayerId: Map<string, any[]>
): PrayerRequest[] {
  const formatted = (prayersData || []).map((prayer) => ({
    ...prayer,
    updates: updatesByPrayerId.get(prayer['id'] as string) || [],
  })) as PrayerRequest[];
  return sortPrayersByLatestActivity(formatted);
}
