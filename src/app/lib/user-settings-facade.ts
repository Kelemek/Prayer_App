...
export class UserSettingsFacade {
  ...
  get tenantContext(): TenantContextService {
    return this.deps.tenantContext;
  }

  set tenantContext(value: TenantContextService) {
    this.deps.tenantContext = value;
  }
  ...
  private requireOnlineForPreferenceChange(actionPhrase: string): boolean {
    return this.deps.connectivity.requireOnline(actionPhrase);
  }
  ...
  setReceiveNotifications(enabled: boolean): void {
    ...
    if (!this.requireOnlineForPreferenceChange('update notification preferences')) {
      return;
    }
    this.receiveNotifications = enabled;
  ...
  setReceivePushNotifications(enabled: boolean): void {
    ...
    if (
      !this.requireOnlineForPreferenceChange('update push notification preferences')
    ) {
      return;
    }
    this.receivePushNotifications = enabled;
  ...
  setBadgeFunctionalityEnabled(enabled: boolean): void {
    ...
    if (!this.requireOnlineForPreferenceChange('update badge preferences')) {
      return;
    }
    this.badgeFunctionalityEnabled = enabled;
  ...
  setMemorizationStrictMode(enabled: boolean): void {
    ...
    if (!this.requireOnlineForPreferenceChange('update memorization practice')) {
      return;
    }
    this.memorizationStrictMode = enabled;
  ...
  setShowPrayForButton(enabled: boolean): void {
    ...
    if (
      !this.requireOnlineForPreferenceChange('update prayer encouragement settings')
    ) {
      return;
    }
    this.showPrayForButton = enabled;
  ...
  applyPersonalPrayerCooldownHours(hours: number): void {
    this.personalPrayerCooldownHours = hours;
    this.personalPrayerCooldownEdited = true;
    void this.savePersonalPrayerCooldownHours();
  }
