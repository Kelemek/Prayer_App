...
export async function runUserSettingsPreferencesLoad(
  ...
  try {
    const { data: subscriberData, error } = await host.deps.supabase.client
      ...
      .select(
        'name, is_active, receive_push, memorization_strict_mode'
      )
      .match(host.deps.membershipPrefs.matchFilter(normalizedEmail))
      .maybeSingle();
    ...
    if (subscriberData) {
      ...
      host.memorizationStrictMode =
        subscriberData.memorization_strict_mode ?? false;
    } else {
      host.receiveNotifications = true;
      host.receivePushNotifications = false;
      host.memorizationStrictMode = false;
