...
**`admin_settings`** (single row `id = 1`) is **platform-global**: the Apple/Android test account and optional client min versions (`min_web_build`, `min_native_version`). Only super-admins can write it (Admin → Tenant Manager). Unset mins mean no force-upgrade; see [client-version-gate.md](client-version-gate.md). In-app feedback is **not** stored here — the `submit-feedback` Edge Function writes to the shared Prayer App Biz **Feedback** Notion database using server-side `NOTION_TOKEN`. If that secret is missing, the function reports `{ configured: false }` and the Angular app hides the feedback UI. Tenant admins cannot change another church’s behavior through this table.
...
- **Email**: Resend API via `send-email` and `trigger-email-processor` Edge Functions. Queue drain uses `/emails/batch` (one recipient per message). Platform `MAIL_SENDER_ADDRESS` / `MAIL_FROM_NAME` are the fallback; per-church From name, local-part, and optional Reply-To live on `tenant_settings` (`get_tenant_mail_identity` / `update_tenant_mail_identity`). Callers pass `tenantId` so Church A does not send as Church B.
- **Admin Auth**: check-admin-status Edge Function (verifies admin status using service role)
- **Rate limiting**: Queue processor pauses between batch requests and honors Resend **429** `Retry-After`
